...
interface ProofRow {
  ...
  platform?: string; // Optional for PGC
  proofUrl: string;
  proofVideo?: string;
  proofImage?: string;
  submissionUrl?: string; // Optional for PGC
  reportedViews?: number; // Optional for PGC
  verifiedViews?: number; // Optional for PGC
...
type CampaignKind = "normal" | "ugc" | "pgc";
...
const DEFAULT_CPMS: Record<CampaignKind, { clipper: number; platform: number }> = {
  ...
  pgc: { clipper: 5000, platform: 2500 },
};
...
const getKind = (row: any): CampaignKind => {
  const k =
    ...
    row?.campaign_type ??
    row?.campaignType;
  return k === "ugc" ? "ugc" : k === "pgc" ? "pgc" : "normal";
};
...
export default function SubmissionsManagement() {
  ...
  const handleApprove = async (e: FormEvent) => {
    ...
    try {
      ...
      const endpoint = isPgc
        ? `${API_BASE}/admin/submissions/${reviewing.submissionId}/proof/${reviewing.proofId}/approve`
        : `${API_BASE}/admin/submissions/${reviewing.submissionId}/proof/${reviewing.proofId}/verify`;
      const payload = isPgc ? { note: adminNote } : { verifiedViews: adjustedViews, note: adminNote };
  ...
  const handleReject = async (e: FormEvent) => {
    ...
    setErr(null);
    try {
      const endpoint = `${API_BASE}/admin/submissions/${reviewing.submissionId}/proof/${reviewing.proofId}/reject`;
      const res = await axios.post(endpoint, { note: adminNote }, { withCredentials: true });
      const updated: ProofRow = res.data;
      setProofs((prev) =>
        prev.map((p) =>
          p.submissionId === updated.submissionId && p.proofId === updated.proofId
  ...
  return (
    <div className="space-y-8">
      ...
      {reviewing && (() => {
        ...
        const clipperReward = kind === "pgc" ? clipper : (adjustedViews * clipper) / 1000;
        const platformFee = kind === "pgc" ? platform : (adjustedViews * platform) / 1000;
              ...
              {kind !== "pgc" && (
                <>
                  <div>
                    ...
                    <input
                      type="number"
                      value={reviewing.reportedViews ?? 0}
                      readOnly
