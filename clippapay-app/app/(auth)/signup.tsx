...
// ── NEW: import the two components for the modal ──────────────────────────────
...
const SignupScreen = () => {
  ...
  // ── NEW: Terms / Privacy modal state ─────────────────────────────────────
  // null = closed | 'terms' = show Terms | 'privacy' = show Privacy Policy
  ...
  useEffect(() => {
    ...
    const detectCountry = async () => {
      try {
        ...
        if (isMounted) setFormData(prev => ({ ...prev, country: 'NG' }));
      } finally {
        if (isMounted) {
  ...
  const handleSignup = async () => {
    ...
    if (!formData.email || !formData.password || !formData.phone) return setError('Please fill in all required fields');
    if (formData.role === 'clipper' && (!formData.firstName || !formData.lastName)) return setError('Please enter your first and last name');
    if (formData.role === 'advertiser' && !formData.contactName) return setError('Please enter contact person name');
    ...
    try {
      setLoading(true);
      const signupData = { ...formData, email: formData.email.toLowerCase() };
  ...
  const getCountryDialCode = (code: string) => COUNTRIES.find(c => c.code === code)?.dialCode || '';
  ...
  return (
    <SafeAreaView style={[styles.safe, styles.container]}>
      ...
      {/* ── Terms / Privacy full-screen modal ──────────────────────────────── */}
      <Modal
      ...
      >
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          ...
          <View style={styles.legalHeader}>
            ...
            <TouchableOpacity onPress={() => setLegalModal(null)} style={styles.legalHeaderClose}>
              <Ionicons name="close" size={24} color="#374151" />
            </TouchableOpacity>
          </View>
          {/* Content */}
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1 }}>
            {legalModal === 'terms' ? <TermsConditions /> : <PrivacyPolicy />}
          </ScrollView>
        </SafeAreaView>
      </Modal>
          ...
          >
            {step === 'form' && (
              <View style={styles.formContainer}>
                ...
                {formData.role === 'clipper' ? (
                  <>
                    <TextInput style={styles.input} placeholder="First Name" placeholderTextColor="#666" onChangeText={t => update('firstName', t)} value={formData.firstName} />
                    <TextInput style={styles.input} placeholder="Last Name" placeholderTextColor="#666" onChangeText={t => update('lastName', t)} value={formData.lastName} />
                  </>
                ) : (
                  <>
                    <TextInput style={styles.input} placeholder="Contact Person Name" placeholderTextColor="#666" onChangeText={t => update('contactName', t)} value={formData.contactName} />
                    <TextInput style={styles.input} placeholder="Company / Brand (optional)" placeholderTextColor="#666" onChangeText={t => update('company', t)} value={formData.company} />
                  </>
                )}

                <TouchableOpacity style={styles.nextButton} onPress={handleSignup} disabled={loading}>
            ...
            {step === 'telegram' && (
              <View style={styles.telegramContainer}>
                ...
                <TouchableOpacity style={styles.nextButton} onPress={() => Linking.openURL(`https://t.me/clippapay?start=${encodedPhone}`)}>
                  <LinearGradient colors={['#2563eb', '#1d4ed8']} style={styles.buttonGradient}>
                    <Text style={styles.buttonText}>Open Telegram</Text>
                  </LinearGradient>
                </TouchableOpacity>

                <TouchableOpacity style={[styles.nextButton, { marginTop: 16 * scale }]} onPress={handleGoToDashboard} disabled={telegramChecking}>
                  <LinearGradient colors={['#16a34a', '#059669']} style={styles.buttonGradient}>
                    {telegramChecking ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>I've Joined → Go to Dashboard</Text>}
                  </LinearGradient>
                </TouchableOpacity>

                <Text style={styles.telegramNote}>Note: You can always join later from your dashboard.</Text>
              </View>
            )}
          </ScrollView>
            ...
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <TouchableOpacity
                      style={[styles.countryItem, formData.country === item.code && styles.countryItemActive]}
...
const styles = StyleSheet.create({
  ...
  otpContainer:      { flex: 1, justifyContent: 'center' },
  telegramContainer: { flex: 1, justifyContent: 'center' },

  title:    { fontSize: 30 * scale, fontWeight: '700', marginBottom: 20 * scale, textAlign: 'center', color: '#fff' },
  subtitle: { textAlign: 'center', marginBottom: 20 * scale, color: '#d1d5db' },
  ...
  passwordContainer: { position: 'relative', marginBottom: 14 * scale },
  passwordInput:     { paddingRight: 50 * scale },
  passwordToggle:    { position: 'absolute', right: 15 * scale, top: 15 * scale, height: 24 * scale, width: 24 * scale, justifyContent: 'center', alignItems: 'center' },
  otpInput:          { textAlign: 'center', fontSize: 24 * scale, letterSpacing: 8 * scale, color: '#000' },

  nextButton:     { height: 55 * scale, borderRadius: 10, overflow: 'hidden', marginTop: 20 * scale },
  buttonGradient: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  buttonText:     { color: '#fff', fontWeight: '700', fontSize: 16 * scale },

  // ── NEW: Legal modal header ──
  legalHeader: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  legalHeaderTitle: { fontSize: 18 * scale, fontWeight: '700', color: '#1F2937' },
  legalHeaderClose: { padding: 4 * scale },

  // ── NEW: Terms text at bottom of form ──
  termsContainer: { marginTop: 20 * scale, paddingHorizontal: 10 * scale },
  termsText:      { fontSize: 12 * scale, color: '#d1d5db', textAlign: 'center', lineHeight: 20 * scale },
  termsLink:      { color: '#9c77db', fontWeight: '700', textDecorationLine: 'underline' },

  // Error / success overlays — unchanged
  errorOverlay:   { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', zIndex: 1000 },
  errorModal:     { backgroundColor: '#fff', borderRadius: 15 * scale, padding: 24 * scale, width: '85%', maxWidth: 400 * scale, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4, elevation: 5 },
  errorHeader:    { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 * scale },
  errorTitle:     { fontSize: 20 * scale, fontWeight: 'bold', color: '#dc2626' },
  closeButton:    { padding: 4 * scale },
  errorModalText: { fontSize: 16 * scale, color: '#374151', lineHeight: 22 * scale, marginBottom: 24 * scale },
  errorOkButton:  { backgroundColor: '#dc2626', borderRadius: 8 * scale, padding: 14 * scale, alignItems: 'center' },
  errorOkButtonText: { color: '#fff', fontSize: 16 * scale, fontWeight: '600' },

  messageOverlay:   { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', zIndex: 1000 },
  messageModal:     { backgroundColor: '#fff', borderRadius: 15 * scale, padding: 24 * scale, width: '85%', maxWidth: 400 * scale, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4, elevation: 5 },
  messageHeader:    { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 * scale },
  messageTitle:     { fontSize: 20 * scale, fontWeight: 'bold', color: '#16a34a' },
  messageModalText: { fontSize: 16 * scale, color: '#374151', lineHeight: 22 * scale, marginBottom: 24 * scale },
  messageOkButton:  { backgroundColor: '#16a34a', borderRadius: 8 * scale, padding: 14 * scale, alignItems: 'center' },
  messageOkButtonText: { color: '#fff', fontSize: 16 * scale, fontWeight: '600' },

  roleContainer:    { flexDirection: 'row', marginBottom: 20 * scale, gap: 10 * scale },
  roleButton:       { flex: 1, padding: 16 * scale, borderRadius: 12, backgroundColor: '#f3f4f6', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#e5e7eb' },
  roleButtonActive: { borderColor: '#7C3AED', borderWidth: 2, backgroundColor: '#ede9fe' },
  roleText:         { fontWeight: '700', fontSize: 16 * scale, color: '#1f2937' },
  roleSubText:      { fontSize: 12 * scale, color: '#6b7280', marginTop: 4 * scale },

  telegramNote: { textAlign: 'center', marginTop: 20 * scale, fontSize: 12 * scale, color: '#d1d5db', fontStyle: 'italic', paddingHorizontal: 20 * scale },

  loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  loadingText:      { marginTop: 16 * scale, fontSize: 16 * scale, color: '#fff' },

  countrySelector:        { backgroundColor: '#f3f4f6', borderRadius: 10, marginBottom: 14 * scale, borderWidth: 1, borderColor: '#e5e7eb' },
  countrySelectorContent: { padding: 14 * scale },
  countrySelectorLabel:   { fontSize: 12 * scale, color: '#6b7280', marginBottom: 4 * scale },
  countrySelectorValue:   { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  countrySelectorText:    { fontSize: 16 * scale, color: '#1f2937', fontWeight: '500' },
  detectingContainer:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 14 * scale, gap: 8 * scale },
  detectingText:          { fontSize: 12 * scale, color: '#9ca3af', fontStyle: 'italic' },

  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' },
  modalContent: { backgroundColor: '#fff', borderTopLeftRadius: 20 * scale, borderTopRightRadius: 20 * scale, maxHeight: '80%', padding: 20 * scale },
  modalHeader:  { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 * scale },
  modalTitle:   { fontSize: 20 * scale, fontWeight: 'bold', color: '#1f2937' },

  countryItem:       { flexDirection: 'row', alignItems: 'center', padding: 16 * scale, borderBottomWidth: 1, borderBottomColor: '#e5e7eb' },
  countryItemActive: { backgroundColor: '#f0fdf4' },
  countryItemName:   { flex: 1, fontSize: 16 * scale, color: '#1f2937' },
  countryItemCode:   { fontSize: 14 * scale, color: '#6b7280', marginRight: 8 * scale },
  countryItemDial:   { fontSize: 14 * scale, color: '#6b7280', marginRight: 8 * scale },

  quickFixButton: { backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 8 * scale, padding: 10 * scale, marginBottom: 14 * scale, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  quickFixText:   { color: '#fff', fontSize: 14 * scale, fontWeight: '500' },
});
