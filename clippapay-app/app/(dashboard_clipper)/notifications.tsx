// app/(dashboard_clipper)/notifications.tsx
...
// ── Icon / colour map (clipper-focused) ──────────────────────────────────────
function getIconAndColor(type: string, priority: string) {
  if (type.includes('payment') || type.includes('wallet_credited')) {
    return { icon: 'cash-outline', color: '#10b981' } as const;
  }
  if (type.includes('withdrawal') || type.includes('deposit')) {
    return { icon: 'arrow-undo-outline', color: '#3b82f6' } as const;
  }
  if (type.includes('offer_received')) {
    return { icon: 'gift-outline', color: '#8b5cf6' } as const;
  }
  if (type.includes('offer_declined') || type.includes('offer_expired')) {
    return { icon: 'close-circle-outline', color: '#ef4444' } as const;
  }
  if (type.includes('revision')) {
    return { icon: 'create-outline', color: '#f59e0b' } as const;
  }
  if (type.includes('submission') || type.includes('clipping')) {
...
// ── Screen ────────────────────────────────────────────────────────────────────
export default function ClipperNotificationsScreen() {
  const {
    notifications,
    unreadCount,
    loading,
  ...
  const handlePress = useCallback(
    async (id: string) => {
      ...
      if (item && !item.read) await markAsRead(id);
      // No navigation — tap marks as read only.
    },
          ...
          loading ? (
            ...
            <View style={styles.centered}>
              ...
              <Text style={styles.emptyBody}>
                Offers, payments, and campaign updates will appear here.
              </Text>
            </View>
          )
        }
