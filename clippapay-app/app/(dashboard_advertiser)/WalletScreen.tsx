...
//  2. Withdrawal body sends correct snake_case fields + currency
//  3. Escrow banner now correctly explains it's pending-withdrawal funds
//  4. Loads platform-details alongside wallet data
//  5. Success feedback in modals before auto-close
...
export default function WalletScreen() {
  ...
  const pickReceipt = async () => {
    ...
    if (status !== 'granted') { Alert.alert('Permission required', 'Allow photo access to attach receipts.'); return; }
  ...
  const submitDeposit = async () => {
    ...
    if (!amt || isNaN(amt) || amt < min) {
      Alert.alert('Invalid amount', `Minimum: ${depositCur === 'NGN' ? fmtNGN(min) : `$${min} USDT`}`);
      return;
    }
    if (!receipt) { Alert.alert('Receipt required', 'Attach your payment receipt or screenshot'); return; }
    if (depositCur === 'USDT' && !txHash.trim()) { Alert.alert('TX Hash required', 'Paste your transaction hash (TXID)'); return; }
    ...
    try {
      ...
      setDepositOk(true);
      setTimeout(() => {
        setDepositModal(false); setDepositAmt(''); setReceipt(null); setTxHash(''); setDepositOk(false);
      }, 2000);
      load(true);
    } catch (e: any) {
      Alert.alert('Error', e.response?.data?.error || 'Could not submit deposit. Try again.');
  ...
  const submitWithdrawal = async () => {
    ...
    if (amt < minW)   { setWdrErr(`Minimum: ${wdrCur === 'NGN' ? fmtNGN(minW) : `$${minW} USDT`}`); return; }
    if (amt > avail)  { setWdrErr(`Exceeds available ${wdrCur} balance (${fmt(avail, wdrCur)})`); return; }
    if (wdrCur === 'NGN' && (!bankName.trim() || !accNum.trim() || !accName.trim())) {
      setWdrErr('Fill in all bank details'); return;
    }
    if (wdrCur === 'USDT' && (!wdrAddr.trim() || !wdrNet.trim())) {
      setWdrErr('Fill in USDT address and network'); return;
    }
    ...
    try {
      ...
      await axios.post(`${API_BASE}/withdrawals`, {
        ...
        // snake_case matches model + backend route
        ...(wdrCur === 'NGN'
      ...
      setWdrErr(e.response?.data?.error || 'Could not submit withdrawal. Try again.');
    } finally { setSubmitting(false); }
  };
    ...
    <View style={{ flex: 1, backgroundColor: '#F5F5F7' }}>
      ...
      >
        ...
        <LinearGradient colors={['#312E81', '#4338CA', '#4F46E5']} style={S.hero}>
          ...
          <Text style={S.heroSub}>Available balances · deposit & withdraw anytime</Text>
        ...
        {(wallet.escrowLocked > 0 || wallet.usdtEscrowLocked > 0) && (
          <View style={S.escrowCard}>
            <View style={S.escrowHeader}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={S.escrowTitle}>Notice</Text>
                <Text style={S.escrowSub}>Funds reserved while admin reviews</Text>
              </View>
            </View>
            <View style={S.escrowChipsRow}>
              {wallet.escrowLocked > 0 && (
                <View style={S.escrowChip}>
                  <Text style={S.escrowChipLabel}>NGN Reserved</Text>
              ...
              {wallet.usdtEscrowLocked > 0 && (
                <View style={[S.escrowChip, { backgroundColor: '#FFFDE7', borderColor: '#FFF176' }]}>
                  <Text style={S.escrowChipLabel}>USDT Reserved</Text>
            ...
            <View style={S.escrowInfoRow}>
              ...
              <Text style={S.escrowInfoTxt}>
                These funds are reserved from your balance while your campaign is in process. If declined or cancelled they are returned automatically.
              </Text>
            </View>
          </View>
        )}
        ...
        <View style={S.section}>
          ...
          {historyItems.length === 0 ? (
            ...
            historyItems.map((item: any) => {
              ...
              const credit   = ['deposit', 'clipping_reward', 'escrow_release', 'refund'].includes(item.type);
              const cur: 'NGN' | 'USDT' = item.currency || 'NGN';
              const label    = item.status === 'completed' ? 'paid' : item.status;
              return (
                <View key={item._id} style={S.txRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={S.txDesc} numberOfLines={1}>
                      {item.description || item.note || (item.type || 'Transaction').replace(/_/g, ' ')}
                    </Text>
                    <Text style={S.txDate}>
                  ...
                  <View style={{ alignItems: 'flex-end', gap: 3 }}>
                    <Text style={[S.txAmt, { color: credit ? '#10B981' : '#EF4444' }]}>
                      {credit ? '+' : '-'}{fmt(item.amount, cur)}
                    </Text>
                    <View style={[S.txStatusPill, { backgroundColor: `${statusColor(item.status)}18` }]}>
                      <Text style={[S.txStatusTxt, { color: statusColor(item.status) }]}>{label}</Text>
                    </View>
                  </View>
                </View>
              );
        ...
        <BlurView intensity={20} tint="dark" style={S.overlay}>
          <View style={S.sheet}>
            ...
            <View style={S.mHdr}>
              ...
              <TouchableOpacity style={S.mClose} onPress={() => { setDepositModal(false); setDepositOk(false); }}>
            ...
            <ScrollView style={{ padding: 20 }} showsVerticalScrollIndicator={false}>
              {depositOk && (
                <View style={{ backgroundColor: '#ECFDF5', borderRadius: 12, padding: 14, marginBottom: 16, alignItems: 'center' }}>
                  <Text style={{ color: '#065F46', fontWeight: '700', fontSize: 14 }}>✅ Deposit submitted! Pending admin approval.</Text>
                </View>
              )}
              ...
              <View style={S.toggle}>
                {(['NGN', 'USDT'] as const).map((c) => (
                  <TouchableOpacity key={c}
                    style={[S.toggleBtn, depositCur === c && S.toggleBtnActive]}
                    onPress={() => { setDepositCur(c); setReceipt(null); setTxHash(''); }}>
              ...
              <View style={S.inputRow}>
                ...
                <TextInput style={S.bigInput} value={depositAmt} onChangeText={setDepositAmt}
                  keyboardType="numeric"
                  placeholder={depositCur === 'NGN' ? '20000' : '50'}
                  placeholderTextColor="#9CA3AF" />
              </View>
              <Text style={S.mHint}>
                Min: {depositCur === 'NGN' ? fmtNGN(MIN.NGN_DEPOSIT) : `$${MIN.USDT_DEPOSIT} USDT`}
              </Text>

              {/* NGN bank details */}
              {depositCur === 'NGN' && (
                <View style={S.detailCard}>
                  <Text style={S.detailCardTitle}>1. Transfer to this account first</Text>
                  {platform ? (
                    ...
                    <Text style={{ color: '#9CA3AF', fontSize: 12, fontStyle: 'italic' }}>Loading bank details…</Text>
                  )}
                  <Text style={[S.mHint, { marginTop: 10 }]}>2. Then upload your receipt below</Text>
                </View>
              )}
              ...
              {depositCur === 'USDT' && (
                <View style={S.detailCard}>
                  ...
                  {platform ? (
                    ...
                    <Text style={{ color: '#9CA3AF', fontSize: 12, fontStyle: 'italic' }}>Loading USDT details…</Text>
                  )}

                  <Text style={[S.mLabel, { marginTop: 14 }]}>2. Paste Transaction Hash (TXID) *</Text>
                  <TextInput style={S.txHashInput} value={txHash} onChangeText={setTxHash}
              ...
              <Text style={[S.mLabel, { marginTop: 16 }]}>
                {depositCur === 'NGN' ? '2. Upload Bank Receipt *' : '3. Upload Screenshot *'}
              </Text>
              <TouchableOpacity style={[S.uploadBox, receipt && { borderColor: '#10B981', backgroundColor: '#ECFDF5' }]} onPress={pickReceipt}>
                ...
                <Text style={[S.uploadTxt, receipt && { color: '#10B981' }]}>
                  {receipt ? (receipt.fileName || 'File attached ✓') : 'Tap to attach receipt'}
                </Text>
              </TouchableOpacity>

              <TouchableOpacity style={[S.submitBtn, (submitting || depositOk) && { opacity: 0.6 }]}
                onPress={submitDeposit} disabled={submitting || depositOk}>
              ...
              <Text style={S.footerNote}>Credited after admin approval</Text>
            </ScrollView>
          </View>
        </BlurView>
      </Modal>
        ...
        <BlurView intensity={20} tint="dark" style={S.overlay}>
          <View style={S.sheet}>
            ...
            <View style={S.mHdr}>
              ...
              <TouchableOpacity style={S.mClose} onPress={() => { setWdrModal(false); setWdrOk(false); setWdrErr(''); }}>
                <Ionicons name="close" size={20} color="#6B7280" />
              </TouchableOpacity>
            </View>
            ...
            <ScrollView style={{ padding: 20 }} showsVerticalScrollIndicator={false}>
              {wdrOk && (
                <View style={{ backgroundColor: '#ECFDF5', borderRadius: 12, padding: 14, marginBottom: 16, alignItems: 'center' }}>
                  <Text style={{ color: '#065F46', fontWeight: '700', fontSize: 14 }}>✅ Request submitted! Processing within 24–48 hours.</Text>
                </View>
              )}
              ...
              <View style={S.toggle}>
                {(['NGN', 'USDT'] as const).map((c) => (
                  <TouchableOpacity key={c}
                    style={[S.toggleBtn, wdrCur === c && S.toggleBtnActive]}
                    onPress={() => { setWdrCur(c); setWdrErr(''); setBankName(''); setAccNum(''); setAccName(''); setWdrAddr(''); setWdrNet(''); }}>
              ...
              <View style={S.inputRow}>
                ...
                <TextInput style={S.bigInput} value={wdrAmt} onChangeText={(v) => { setWdrAmt(v); setWdrErr(''); }}
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 6, marginBottom: 16 }}>
                <Text style={S.mHint}>Min: {wdrCur === 'NGN' ? fmtNGN(MIN.NGN_WITHDRAWAL) : `$${MIN.USDT_WITHDRAWAL} USDT`}</Text>
                <Text style={[S.mHint, { color: '#4F46E5', fontWeight: '700' }]}>
              ...
              {wdrCur === 'NGN' ? (
                <>
                  {([
                    ...
                    ['Account Number', accNum,    setAccNum,   '10-digit NUBAN',           'numeric'],
                    ['Account Name',   accName,   setAccName,  'Exact account holder name','default'],
                  ] as any[]).map(([lbl, val, setter, ph, kb]) => (
                    <View key={lbl} style={{ marginBottom: 14 }}>
                      <Text style={S.mLabel}>{lbl}</Text>
                      <TextInput style={S.fieldInput} value={val}
                        onChangeText={(v: string) => { setter(v); setWdrErr(''); }}
                        placeholder={ph} placeholderTextColor="#9CA3AF" keyboardType={kb} />
                    </View>
                  ))}
                </>
              ) : (
                <>
                  <View style={{ marginBottom: 14 }}>
                    ...
                    <TextInput style={S.fieldInput} value={wdrAddr}
                      onChangeText={(v) => { setWdrAddr(v); setWdrErr(''); }}
                      placeholder="Your USDT address" placeholderTextColor="#9CA3AF" />
                  </View>
                  <View style={{ marginBottom: 14 }}>
                    <Text style={S.mLabel}>Network</Text>
                    <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
                      {['TRC20', 'ERC20', 'BEP20'].map((net) => (
                        <TouchableOpacity key={net}
                          style={{ flex: 1, paddingVertical: 10, borderRadius: 10, alignItems: 'center', borderWidth: 1.5,
                            borderColor: wdrNet === net ? '#4F46E5' : '#E5E7EB',
                            backgroundColor: wdrNet === net ? '#EEF2FF' : '#FAFAFA' }}
                          onPress={() => { setWdrNet(net); setWdrErr(''); }}>
                          <Text style={{ fontSize: 13, fontWeight: '700', color: wdrNet === net ? '#4F46E5' : '#6B7280' }}>{net}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                    <TextInput style={S.fieldInput} value={wdrNet}
                      onChangeText={(v) => { setWdrNet(v); setWdrErr(''); }}
                      placeholder="or type network manually" placeholderTextColor="#9CA3AF" />
                  </View>
                </>
              )}

              {/* Info */}
              <View style={{ flexDirection: 'row', backgroundColor: '#EFF6FF', borderRadius: 10, padding: 10, gap: 8, marginBottom: 4 }}>
                <Ionicons name="information-circle-outline" size={14} color="#3B82F6" />
                <Text style={{ flex: 1, fontSize: 12, color: '#1E40AF', lineHeight: 17 }}>
                  Funds are reserved from your balance immediately. Returned automatically if declined.
                </Text>
              </View>

              <TouchableOpacity style={[S.submitBtn, { marginTop: 20 }, (submitting || wdrOk) && { opacity: 0.6 }]}
                onPress={submitWithdrawal} disabled={submitting || wdrOk}>
...
function BalanceCard({ label, amount, sub, iconName, accentColor, subColor, onDeposit, onWithdraw }: {
  ...
  return (
    <View style={BC.card}>
      ...
      <Text style={[BC.sub, { color: subColor }]} numberOfLines={2}>{sub}</Text>
      <View style={BC.btnRow}>
        <TouchableOpacity style={BC.fundBtn} onPress={onDeposit}>
          <Ionicons name="add" size={10} color="#FFF" />
          <Text style={BC.fundTxt}>Deposit</Text>
        </TouchableOpacity>
        <TouchableOpacity style={BC.wdrBtn} onPress={onWithdraw}>
          <Ionicons name="arrow-up" size={10} color="#4338CA" />
...
const BC = StyleSheet.create({
  ...
  fundTxt: { fontSize: 8, fontWeight: '700', color: '#FFF' },
  ...
  wdrTxt:  { fontSize: 8, fontWeight: '700', color: '#4338CA' },
});
...
const S = StyleSheet.create({
  ...
  escrowCard:     { margin: 16, backgroundColor: '#FFF', borderRadius: 20, padding: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 8, elevation: 3 },
  ...
  mHint:    { fontSize: 11, color: '#9CA3AF', marginTop: 5 },
